...
class JUser extends Base {
    ...
    public function execute($pageVars) {
      ...
      $extraParams = $pageVars["route"]["extraParams"];
        ...
        if ($action=="list") {
            $jUserModel = new \Model\JUser\Listing($extraParams);
            $this->content["result"] = $jUserModel->askWhetherToGetUserList();
            $this->content["output-format"] = $jUserModel->outputFormat;
            return array ("type"=>"view", "view"=>"jUserList", "pageVars"=>$this->content); }
